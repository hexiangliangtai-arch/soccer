...
import { clamp, moveToward } from './pitchMath'
import { goalkeeperTarget, teamTactic } from './playerStateEngine'
import { chooseZoneTarget, detectTacticalPhase, separateTeammates, updatePlayerZone } from './zoneEngine'
...
function targetForState(player:MatchPlayerState,world:MatchWorldState):PitchPosition {
  ...
  if(player.actionState==='cover'||player.actionState==='support'||player.actionState==='runIntoSpace'||player.actionState==='dribble') {
    return chooseZoneTarget(player,world)
  }
  if(player.actionState==='carryBall')return chooseZoneTarget(player,world)
  return chooseZoneTarget(player,world)
}

export function updateContinuousMovement(world:MatchWorldState) {
  const refreshZones=world.stepIndex===0||world.stepIndex%15===0
  if(refreshZones) {
    const phases={home:detectTacticalPhase(world,'home'),away:detectTacticalPhase(world,'away')}
    world.players.forEach((player)=>{player.currentZone=updatePlayerZone(player,world,phases[player.team])})
  }
  const moved=world.players.map((player)=>{
  ...
  world.players=refreshZones?separateTeammates(moved):moved
}
